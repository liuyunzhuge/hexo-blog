...
```
这个`genComponentModel`，并不像标准表单元素一样直接就包含了`addHandler`和`addProp`的逻辑，所以从这两个文件看不出，v-model在自定义组件下是否使用了`value`prop和`input`事件。仅仅能看到的是，它把核心的数据存储到了自定义组件的model属性上：
```js
  el.model = {
    value: `(${value})`,
    expression: JSON.stringify(value),
...
```
`options.model`中的`options`就是Vue实例构造时传递的options。所以，**自定义组件使用v-model需要`value`prop和`input`事件，是可以更改的**，在本篇最后的内容里面，会用到这个特性，来实现更加完美的自定义checkbox。
...
```
这是官网的一个实现，我认为是不够的，因为它连标准的checkbox该有的东西都不全，比如说`true-value`和`false-value`，比如说`v-model`绑定数组。 要想实现在标准checkbox元素基础之上的自定义checkbox，应该参考v-model对标准checkbox元素的处理来写。 举例如下：
